...
import type {
  Estagiario, DocumentoDigitalizado, Filho, FormacaoEducacional,
  HistoricoCargoEntry, HistoricoSalarioEntry, IdiomaFalado,
  EstagiarioStatusType, Suspensao,
} from '../../types'
import {
  CONTA_TIPO_LABEL, EMPRESA_OPTIONS, ESTADO_CIVIL_LABEL, FORMACAO_LABEL,
  GENERO_LABEL, NIVEL_IDIOMA_LABEL, PIX_TIPO_LABEL, ESTAGIARIO_STATUS_LABEL,
  RACA_LABEL, SUSPENSAO_TIPO_LABEL, TIPO_PAGAMENTO_LABEL,
  TIPO_UNIAO_LABEL,
} from '../../types'

// Página de detalhe do estagiário. Layout em tabs com
...
export default function EstagiarioDetalhe() {
  const { id } = useParams<{ id: string }>()
  const navigate = useNavigate()
  const [p, setP] = useState<Estagiario | null>(null)
  const [loading, setLoading] = useState(true)
  const [tab, setTab] = useState<TabKey>('principal')

  useEffect(() => {
    if (!id) return
    const u = onSnapshot(doc(db, 'estagiarios', id), (snap) => {
      if (!snap.exists()) { setP(null); setLoading(false); return }
      setP({ id: snap.id, ...(snap.data() as Omit<Estagiario, 'id'>) })
      setLoading(false)
    }, () => setLoading(false))
    return u
  }, [id])
      ...
      <>
        ...
        <div className="content">
          <div className="empty">
            ...
            <div className="empty-ttl">Estagiário não encontrado</div>
            <div className="empty-sub">
              <Link to="/dp/estagiarios" style={{ color: 'var(--g600)' }}>← Voltar para lista de estagiários</Link>
            </div>
          </div>
        </div>
      </>
  ...
  return (
    <>
      <Topbar
        ...
        icon="◉"
        actions={
          <button className="tbtn" onClick={() => navigate('/dp/estagiarios')}>← Voltar</button>
        }
      />
      <div className="content">
        <EstagiarioHeader p={p} />
...
// ─────────────────────────── Header com foto ───────────────────────────
function EstagiarioHeader({ p }: { p: Estagiario }) {
  ...
  async function uploadFoto(file: File) {
    ...
    try {
      const r = await uploadFile(file, `estagiarios/${p.id}/foto`)
      await updateDoc(doc(db, 'estagiarios', p.id), {
  ...
  // Mapeamento de status do estagiário pra cor da badge no header.
  // Estagiário não tem 'férias' / 'afastado' (só prestador), então só
  // distinguimos contrato suspenso (amarelo), efetivado (info) e desligado/finalizado (vermelho).
  const statusBdg = p.status === 'ativo' ? 'ok'
    : p.status === 'contrato_suspenso' ? 'warn'
    : p.status === 'efetivado' ? 'info'
    : 'bad'
    ...
    <div className="panel" style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
      ...
      <div style={{ flex: 1, minWidth: 200 }}>
        ...
        <div style={{ fontSize: 13, color: 'var(--mut)' }}>
          {p.curso || p.area} · {p.area} · {p.empresa}
        </div>
        <div style={{ fontSize: 11, color: 'var(--mut)', marginTop: 4 }}>
          Estágio · Início {fmtDate(p.dataInicio)} · Término {fmtDate(p.dataTermino)}
          {p.gestorNome && <> · Gestor: {p.gestorNome}</>}
        </div>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
        <span className={`bdg ${statusBdg}`} style={{ fontSize: 12 }}>{ESTAGIARIO_STATUS_LABEL[p.status]}</span>
        {p.indicadoPorNome && (
...
// ─────────────────────────── Tab Principal ───────────────────────────
function TabPrincipal({ p }: { p: Estagiario }) {
  ...
  return (
    <div>
      ...
      <div className="form-grid">
        ...
        <Field label="Curso">{p.curso || '—'}</Field>
        <Field label="Instituição">{p.instituicao || '—'}</Field>
        <Field label="Semestre">{p.semestre || '—'}</Field>
        <Field label="Área">{p.area}</Field>
        <Field label="Mentor">{p.mentor || '—'}</Field>
        <Field label="Empresa do grupo">{p.empresa}</Field>
        <Field label="Regime">Estágio</Field>
        <Field label="Bolsa-auxílio">{fmtMoney(p.bolsa)}</Field>
        <Field label="Início do estágio">{fmtDate(p.dataInicio)}</Field>
        <Field label="Término do estágio">{fmtDate(p.dataTermino)}</Field>
...
function FormPrincipal({ p, onClose }: { p: Estagiario; onClose: () => void }) {
  ...
  const [genero, setGenero] = useState<Estagiario['genero'] | ''>(p.genero ?? '')
  const [raca, setRaca] = useState<Estagiario['raca'] | ''>(p.raca ?? '')
  ...
  // No estágio o "cargo" é a área de atuação (não tem cargo separado).
  // Mantemos o nome `cargo` aqui pra reaproveitar o histórico imutável
  // de cargo (HistoricoCargoEntry) se a área mudar.
  const [area, setArea] = useState(p.area)
  const [curso, setCurso] = useState(p.curso ?? '')
  const [instituicao, setInstituicao] = useState(p.instituicao ?? '')
  const [semestre, setSemestre] = useState(p.semestre ?? '')
  const [mentor, setMentor] = useState(p.mentor ?? '')
  const [empresa, setEmpresa] = useState(p.empresa)
  const [bolsa, setBolsa] = useState<number | ''>(typeof p.bolsa === 'number' ? p.bolsa : '')
  const [dataIni, setDataIni] = useState(toDateInput(p.dataInicio))
  const [dataTerm, setDataTerm] = useState(toDateInput(p.dataTermino))
  const [status, setStatus] = useState<EstagiarioStatusType>(p.status)
  ...
  const [bolsaMotivo, setBolsaMotivo] = useState('')
  const [areaMotivo, setAreaMotivo] = useState('')
  const [saving, setSaving] = useState(false)
  const [err, setErr] = useState<string | null>(null)

  const areaMudou = area.trim() !== (p.area ?? '').trim()
  const bolsaMudou = (typeof bolsa === 'number' ? bolsa : null) !== (typeof p.bolsa === 'number' ? p.bolsa : null)
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const payload: Record<string, unknown> = {
        ...
        dataInicio: dataIni ? Timestamp.fromDate(new Date(dataIni + 'T00:00:00')) : null,
        dataTermino: dataTerm ? Timestamp.fromDate(new Date(dataTerm + 'T00:00:00')) : null,
      ...
      if (areaMudou) {
        const entry: HistoricoCargoEntry = {
          id: genId(),
          cargoAnterior: p.area,
          cargoNovo: area,
          motivo: areaMotivo || undefined,
          vigenciaEm: nowTs,
          registradoEm: nowTs,
      ...
      if (bolsaMudou && typeof bolsa === 'number') {
        const entry: HistoricoSalarioEntry = {
          id: genId(),
          salarioAnterior: typeof p.bolsa === 'number' ? p.bolsa : undefined,
          salarioNovo: bolsa,
          motivo: bolsaMotivo || undefined,
          vigenciaEm: nowTs,
          registradoEm: nowTs,
          registradoPorUid: byUid,
          registradoPorNome: byName,
      ...
      await updateDoc(doc(db, 'estagiarios', p.id), updates)
      onClose()
    } catch (e2) {
  ...
  return (
    <form onSubmit={handleSubmit} className="row-gap-14">
      ...
      <div className="form-grid">
        ...
        <div className="field">
          <label>Gênero</label>
          <select value={genero} onChange={(e) => setGenero(e.target.value as Estagiario['genero'])}>
        ...
        <div className="field">
          ...
          <select value={raca} onChange={(e) => setRaca(e.target.value as Estagiario['raca'])}>
            <option value="">— selecione —</option>
        ...
        <div className="field"><label>Curso</label><input value={curso} onChange={(e) => setCurso(e.target.value)} /></div>
        <div className="field"><label>Instituição</label><input value={instituicao} onChange={(e) => setInstituicao(e.target.value)} /></div>
        <div className="field"><label>Semestre</label><input value={semestre} onChange={(e) => setSemestre(e.target.value)} placeholder="Ex.: 5º" /></div>
        <div className="field"><label>Área *</label><input value={area} onChange={(e) => setArea(e.target.value)} required /></div>
        {areaMudou && (
          <div className="field" style={{ gridColumn: '1 / -1' }}>
            <label>Motivo da mudança de área (vai pro histórico)</label>
            <input value={areaMotivo} onChange={(e) => setAreaMotivo(e.target.value)} placeholder="Ex.: realocação de time…" />
          </div>
        )}
        ...
        <div className="field">
          ...
          <input type="number" min={0} step={0.01} value={bolsa} onChange={(e) => setBolsa(e.target.value === '' ? '' : Number(e.target.value))} />
        </div>
        {bolsaMudou && (
          <div className="field" style={{ gridColumn: '1 / -1' }}>
            <label>Motivo da mudança de bolsa (vai pro histórico)</label>
            <input value={bolsaMotivo} onChange={(e) => setBolsaMotivo(e.target.value)} placeholder="Ex.: aumento por desempenho…" />
          </div>
        )}
        <div className="field"><label>Início do estágio *</label><input type="date" value={dataIni} onChange={(e) => setDataIni(e.target.value)} required /></div>
        <div className="field"><label>Término do estágio *</label><input type="date" value={dataTerm} onChange={(e) => setDataTerm(e.target.value)} required /></div>
        <div className="field">
          <label>Status</label>
          <select value={status} onChange={(e) => setStatus(e.target.value as EstagiarioStatusType)}>
            {Object.entries(ESTAGIARIO_STATUS_LABEL).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
          </select>
        </div>
...
// ─────────────────────────── Tab Endereço ───────────────────────────
function TabEndereco({ p }: { p: Estagiario }) {
  const e = p.endereco ?? {}
  const [editing, setEditing] = useState(false)
...
// ─────────────────────────── Tab Bancários ───────────────────────────
function TabBancarios({ p }: { p: Estagiario }) {
...
// ─────────────────────────── Tab Família ───────────────────────────
function TabFamilia({ p }: { p: Estagiario }) {
  const f = p.familia ?? {}
  const [editing, setEditing] = useState(false)
...
function TabEscolaridade({ p }: { p: Estagiario }) {
...
// ─────────────────────────── Tab Documentos ───────────────────────────
function TabDocumentos({ p }: { p: Estagiario }) {
  const { profile } = useAuth()
  ...
  async function up(file: File) {
    ...
    setUploading(true); setErr(null)
    try {
      const r = await uploadFile(file, `estagiarios/${p.id}/documentos`)
...
// ─────────────────────────── Tab Históricos ───────────────────────────
function TabHistoricos({ p }: { p: Estagiario }) {
...
// ─────────────────────────── Tab Suspensões ───────────────────────────
function TabSuspensoes({ p }: { p: Estagiario }) {
